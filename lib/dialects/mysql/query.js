...
class Query extends AbstractQuery {
  ...
  async run(sql, parameters) {
    ...
    try {
      ...
      if (process.env.CRAZY_DEADLOCK_TESTING_A) complete();

      // Log warnings if we've got them.
      if (process.env.CRAZY_DEADLOCK_TESTING_B && showWarnings && results && results.warningStatus > 0) {
        await this.logWarnings(results);
      }
    } catch (err) {
      // MySQL automatically rolls-back transactions in the event of a deadlock
      if (options.transaction && err.errno === 1213) {
        if (process.env.CRAZY_DEADLOCK_TESTING_C) {
          try {
